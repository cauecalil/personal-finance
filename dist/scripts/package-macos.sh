...
PORTABLE_SOURCE_PATH="$OUTPUT_DIR/$APP_NAME.app"
...
PORTABLE_ARCHIVE_PATH="$OUTPUT_DIR/$PORTABLE_DIR_NAME-$APP_VERSION-macos-portable.zip"
if [[ -f "$PORTABLE_ARCHIVE_PATH" ]]; then
  rm -f "$PORTABLE_ARCHIVE_PATH"
fi

ditto -c -k --sequesterRsrc --keepParent "$PORTABLE_SOURCE_PATH" "$PORTABLE_ARCHIVE_PATH"
